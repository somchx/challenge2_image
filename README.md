...
วัตถุ/ฉากที่ incompatible กับ feature-point matching โดยพื้นฐาน — ระบบควร fail ตามที่คาดไว้

### f1 — Crowded Pedestrians on Street

| Output frame | Output GIF |
|---|---|
| ![f1 detected](outputs/expected_fail/output_f1.png) | ![f1 gif](outputs/expected_fail/output_f1.gif) |

**ผลที่คาดหวัง:** SIFT เจอ keypoint กว่า 9,000 จุดกระจายทั่วตึก ป้าย และเสื้อผ้าทุกคน ระบบแยกไม่ออกว่าจุดไหนคือ "คนเดิม" นอกจากนี้ homography เดี่ยวไม่สามารถ represent คนหลายคนที่เคลื่อนไหวอิสระและบังกันได้ → ไม่ detect
...
**ผลที่คาดหวัง:** รถเคลื่อนเข้า-ออกกล้องอย่างรวดเร็ว perspective เปลี่ยนแบบ 3D เกินกว่า SIFT จะรับไหว รถที่อยู่ไกลมี keypoint น้อยเกินไป ราวกันตกและต้นไม้ข้างทางสร้าง false match จำนวนมาก → ไม่ detect
...
**ผลที่คาดหวัง:** ธงโบกสะบัดเป็น non-rigid deformation — บิด พับ และบังตัวเองตลอดเวลา homography ต้องการวัตถุแบนและแข็ง ลายบนธงหายไปตามรอยพับทำให้ correspondence ขาดช่วง → ไม่ detect

---

### f4 — Underwater Shark Scene

| Output frame | Output GIF |
|---|---|
| ![f4 detected](outputs/expected_fail/output_f4.png) | ![f4 gif](outputs/expected_fail/output_f4.gif) |

**ผลที่คาดหวัง:** น้ำทำให้ภาพเบลอ edge หาย; แสงสีแดงถูกดูดซับทำให้ฉลามกลืนไปกับพื้นหลังสีน้ำเงิน (camouflage); เม็ดทรายและฟองอากาศถูก detect เป็น keypoint ปลอมจำนวนมาก → ไม่ detect

---

### f5 — Pouring Water into Glass

| Output frame | Output GIF |
|---|---|
| ![f5 detected](outputs/expected_fail/output_f5.png) | ![f5 gif](outputs/expected_fail/output_f5.gif) |

**ผลที่คาดหวัง:** แก้วน้ำใสไม่มี texture ของตัวเอง keypoint (~1,300 จุด) มาจาก specular reflection และ background ที่หักเหผ่านแก้ว ซึ่งเปลี่ยนตามมุมกล้องและปริมาณน้ำตลอดเวลา → descriptor matching เป็นไปไม่ได้ → ไม่ detect
